...
func _on_door_highlights_started() -> void:
	radarmast_console_cap.material_overlay = CONST.HIGHLIGHT_MAT
...
func _on_door_interacted() -> void:
	if is_open:
		return
	var t := create_tween()
	t.tween_property(radarmast_console_cap, "rotation_degrees:x", 100.0, 2.0).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
	is_open = true
	$DoorInteraction.block = true
	$LeverInteraction.block = false

func _on_lever_highlights_started() -> void:
	$radar/RadarmastLever.material_overlay = CONST.HIGHLIGHT_MAT
...
func _on_lever_interacted() -> void:
	...
	t.tween_property($radar/RadarmastTop, "position:y", 0.0, 5.0).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
	t.set_parallel().tween_property($radar/RadarmastTop/RadarmastTop2, "position:y", 4.0, 5.0).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
